...
const importStatement = 'import { getBaseGearNameTranslation } from "@/src/data/translated-affixes/base-gear-name-translations";\nimport { getTranslatedAffixText } from "@/src/lib/affix-translator";\n';
...
filesToCheck.forEach(file => {
  ...
  const hasTranslationImport = content.includes('getBaseGearNameTranslation') || content.includes('getTranslatedAffixText');
...
console.log('  - `${name} — ${stats}` → `${getBaseGearNameTranslation(name)} — ${getTranslatedAffixText(stats)}`');
