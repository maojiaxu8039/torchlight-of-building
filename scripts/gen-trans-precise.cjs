...
function parseAffixes(html) {
  ...
  while ((tdMatch = tdRegex.exec(html)) !== null) {
    ...
    // 只提取到第一个 </span> 之前的内容
    let text = tdContent.replace(/data-modifier-id="\d+"/, '').trim();
    
    // 提取 text-mod span 内的内容
    const textModMatch = text.match(/<span[^>]*class="text-mod"[^>]*>([\s\S]*?)<\/span>/i);
    if (textModMatch) {
      text = textModMatch[1];
    } else {
      // 如果没有 text-mod，提取到第一个 </span> 之前
      const spanMatch = text.match(/^([\s\S]*?)<\/span>/i);
      if (spanMatch) {
        text = spanMatch[1];
      }
    }
