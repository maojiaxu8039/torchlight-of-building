...
async function extractTranslations() {
  ...
  try {
    ...
    const pattern = /data-modifier-id="([^"]+)"[^]*?<td[^>]*>([\s\S]*?)<\/td>(?=[^]*?<\/tr>)/gi;

    let match;
    while ((match = pattern.exec(enHtml)) !== null) {
      const id = match[1];
      const tdContent = match[2];
      const text = tdContent.replace(/<[^>]+>/g, ' ').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    console.log('\nSample translations:');
    let count = 0;
    Object.entries(translations).slice(0, 10).forEach(([en, cn]) => {
