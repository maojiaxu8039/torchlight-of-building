...
const content = fs.readFileSync("src/data/translated-affixes/complete-affix-translations.ts", "utf8");
const jsonMatch = content.match(/export const AFFIX_NAME_TRANSLATIONS[^=]*=\s*(\{[\s\S]*\});?\s*$/);
...
ranges.forEach(function(range) {
  avoidRanges.forEach(function(avoidRange) {
    const en = "+" + range + "% Elemental Resistance +" + avoidRange + "% chance to avoid Elemental Ailment";
...
const newContent = "export const AFFIX_NAME_TRANSLATIONS: Record<string, string> = " + JSON.stringify(translations, null, 2) + ";";
fs.writeFileSync("src/data/translated-affixes/complete-affix-translations.ts", newContent);
console.log("Done!");
