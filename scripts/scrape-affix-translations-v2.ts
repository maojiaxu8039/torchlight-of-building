...
function parseModifiers(html: string): ModifierData {
  ...
  while ((trMatch = trRegex.exec(html)) !== null) {
    const tier = trMatch[1];
    const rowContent = trMatch[2];

    const idMatch = rowContent.match(/data-modifier-id="(\d+)"/);
    if (!idMatch) continue;

    const modifierId = idMatch[1];

    const spanMatch = rowContent.match(
      /<span[^>]*data-modifier-id="\d+"[^>]*>([\s\S]*?)<\/span>([^<]*)/i,
    );
    if (!spanMatch) continue;

    let valueText = spanMatch[1].replace(/<[^>]+>/g, "");
    valueText = valueText.replace(/&ndash;/g, "-");
    valueText = valueText.replace(/&amp;/g, "&");

    let fullText = valueText + spanMatch[2];
...
async function main() {
  ...
  const incompleteByTier: { tier: string; en: string; cn?: string }[] = [];
    ...
    try {
      ...
      Object.entries(result.enModifiers).forEach(([id, enData]) => {
        if (result.cnModifiers[id]) {
          ...
          incompleteByTier.push({ tier: enData.tier, en: enData.text });
        }
      });
  ...
  console.log(`   Incomplete (EN only): ${incompleteByTier.length}`);
