...
const MISSING_TYPES = [
  ...
  { type: "INT_Chest", url: "???" },
  { type: "STR_Chest", url: "???" },
  { type: "STR_OneHandedAxe", url: "???" },
  { type: "STR_OneHandedHammer", url: "???" },
  { type: "STR_OneHandedSword", url: "???" },
  { type: "STR_TwoHandedAxe", url: "???" },
  { type: "STR_TwoHandedHammer", url: "???" },
  { type: "STR_TwoHandedSword", url: "???" },
];
...
async function scrapeEquipmentType(config) {
  console.log(`\n📦 Scraping: ${config.type} (${config.url})`);
...
async function main() {
  ...
  const scrapedFile = path.join(__dirname, "scraped-translations-v2.json");
  fs.writeFileSync(
    scrapedFile,
  ...
  const enOnlyFile = path.join(__dirname, "en-only-entries-v2.json");
  ...
  const cnOnlyFile = path.join(__dirname, "cn-only-entries-v2.json");
  ...
  if (allEnOnly.length > 0) {
    ...
    if (allEnOnly.length > 30) {
      console.log(
        `   ... and ${allEnOnly.length - 30} more (see en-only-entries-v2.json)`,
      );
    }
  } else {
  ...
  if (allCnOnly.length > 0) {
    ...
    if (allCnOnly.length > 30) {
      console.log(
        `   ... and ${allCnOnly.length - 30} more (see cn-only-entries-v2.json)`,
      );
    }
  } else {
    console.log("   None!");
  ...
  console.log("\n" + "=".repeat(60));
  console.log("\n❓ Missing URL formats (need to be confirmed):");
  MISSING_TYPES.forEach((config) => {
    console.log(`   ${config.type}: "${config.url}" - NOT FOUND ON WEBSITE`);
  });
}
