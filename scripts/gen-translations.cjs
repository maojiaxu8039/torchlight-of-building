...
function getCnUrl(filename) {
  const name = filename.replace(".html", "");
  return CN_URL_MAP[name] || name;
}
...
function parseAffixes(html) {
  ...
  while ((tdMatch = tdRegex.exec(html)) !== null) {
    ...
    if (idMatch) {
      ...
      currentTier = tierMatch ? tierMatch[1] : "";
...
async function main() {
  ...
  for (const file of files) {
    ...
    const cnUrl = `${BASE_URL}/${encodeURIComponent(getCnUrl(file))}`;
