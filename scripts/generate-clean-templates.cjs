...
async function main() {
  ...
  Object.entries(templates.Prefix).forEach((entry) => {
  ...
  Object.entries(templates.Suffix).forEach((entry) => {
    console.log("  '" + entry[0] + "': '" + entry[1] + "',");
  });

  // 保存到文件
  fs.writeFileSync(
    "scripts/affix-clean-templates.json",
