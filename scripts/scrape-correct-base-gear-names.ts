...
async function scrapeAllBaseGearNames(): Promise<void> {
  ...
  for (const equipmentType of EQUIPMENT_TYPES) {
    try {
      ...
      if (baseGears.length > 0) {
        ...
        const outputFile = path.join(outputDir, `correct-${equipmentType.toLowerCase().replace(/[()]/g, '')}.json`);
  ...
  const combinedOutputFile = path.join(outputDir, 'correct-all-base-gear-translations.json');
  ...
  const uniqueOutputFile = path.join(outputDir, 'correct-unique-base-gear-translations.json');
  fs.writeFileSync(
    uniqueOutputFile,
...
`;
  ...
  const tsOutputFile = path.join(outputDir, 'correct-base-gear-name-translations.ts');
