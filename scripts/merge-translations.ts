...
const oldData = JSON.parse(
  fs.readFileSync(path.join(outputDir, 'unique-base-gear-translations.json'), 'utf-8')
);

const newData = JSON.parse(
  fs.readFileSync(path.join(outputDir, 'correct-unique-base-gear-translations.json'), 'utf-8')
);
...
const sortedEntries = Object.entries(fixedMerged).sort((a, b) => a[0].localeCompare(b[0]));
