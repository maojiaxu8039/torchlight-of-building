...
async function scrapeAllBaseGearNames(): Promise<void> {
    ...
    try {
      ...
      if (baseGears.length > 0) {
        ...
        const outputFile = path.join(
          outputDir,
          `complete-${equipmentType.toLowerCase().replace(/[()]/g, "")}.json`,
        );
  ...
  const combinedOutputFile = path.join(
    outputDir,
    "complete-all-base-gear-translations.json",
  );
  fs.writeFileSync(
    combinedOutputFile,
  ...
  const uniqueOutputFile = path.join(
    outputDir,
    "complete-unique-base-gear-translations.json",
  );
  fs.writeFileSync(
    uniqueOutputFile,
...
`;
  ...
  const tsOutputFile = path.join(
    outputDir,
    "complete-base-gear-name-translations.ts",
  );
