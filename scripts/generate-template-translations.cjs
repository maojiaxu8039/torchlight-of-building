  ...
  try {
    ...
    Object.entries(enById).forEach((entry) => {
      ...
      if (cnText && enText !== cnText) {
        ...
        // 生成模板：移除数值范围，保留词缀名称
        const template = enText
          .replace(/\+\(\d+-\d+\)/g, "+(--)")
          .replace(/\+\d+-\d+/g, "+--");
        if (!templates[data.type]) templates[data.type] = {};

        // 提取词缀名称部分
        const namePart = template
          .replace(/\+\(--\)/g, "")
          .replace(/\+--/g, "")
          .trim();
        templates[data.type][namePart] = cnText
          .replace(/\+\(--\)/g, "")
          .replace(/\+--/g, "")
          .trim();
      }
...
async function main() {
  ...
  for (let i = 0; i < testEquipments.length; i++) {
    ...
    Object.entries(result.templates).forEach((entry) => {
      const type = entry[0];
  ...
  Object.entries(allTemplates.Prefix).forEach((entry) => {
  ...
  Object.entries(allTemplates.Suffix).forEach((entry) => {
    console.log("  " + entry[0] + " -> " + entry[1]);
  });

  // 保存模板翻译到文件
  fs.writeFileSync(
    "scripts/affix-templates.json",
