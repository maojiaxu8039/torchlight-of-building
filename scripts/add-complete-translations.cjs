...
const translations = JSON.parse(fs.readFileSync("src/data/translated-affixes/complete-affix-translations.ts", "utf8").replace(/^export const AFFIX_NAME_TRANSLATIONS[^=]*=\s*{/s, "").replace(/^\s*};/s, ""));
...
let added = 0;
Object.entries(complete).forEach(function(entry) {
...
fs.writeFileSync("src/data/translated-affixes/complete-affix-translations.ts", "export const AFFIX_NAME_TRANSLATIONS: Record<string, string> = " + JSON.stringify(translations, null, 2) + ";");
