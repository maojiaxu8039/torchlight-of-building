...
function parseAffixes(html) {
  ...
  while ((tdMatch = tdRegex.exec(html)) !== null) {
    ...
    // 只提取到第一个 </span> 之前的内容
    let text = tdContent;
    const spanMatch = text.match(/data-modifier-id="(\d+)"[^>]*>([\s\S]*?)<\/span>/i);
    if (spanMatch) {
      text = spanMatch[2];
    } else {
      // 如果没有 </span>，提取到第一个 <i 或 <div 之前
      text = text.replace(/data-modifier-id="(\d+)"[^>]*>/i, '');
      const iMatch = text.match(/^([\s\S]*?)(?=<i|<div|$)/i);
      if (iMatch) {
        text = iMatch[1];
      }
    }
