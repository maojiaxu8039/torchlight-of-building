...
function parseAffixes(html) {
  ...
  while ((idMatch = idRegex.exec(html)) !== null) {
    ...
    text = text
      ...
      .replace(/<span[^>]*class="[^"]*text-mod[^"]*"[^>]*>/gi, '')  // 移除 text-mod span 开始标签
      .replace(/<span[^>]*>/gi, '')  // 移除其他 span 开始标签
    ...
    // 移除末尾可能残留的 tooltip 内容（如 "腰部 需求等级 1"）
