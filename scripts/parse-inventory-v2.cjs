...
async function parseInventory() {
  ...
  const gearLinks = uniqueLinks.filter((l) => {
    ...
    return (
      slug.match(
        /^(Claw|Sword|Axe|Hammer|Staff|Rod|Wand|Bow|Crossbow|Gun|Pistol|Musket|Cannon|Dagger|Scythe|Maul|Spear)/i,
      ) ||
      slug.match(
        /_Helmet|_Gloves|_Boots|_Belt|_Ring|_Necklace|_Shield|_One_Handed|_Two_Handed/i,
      )
    );
  });
  ...
  uniqueLinks.forEach((link, idx) => {
    ...
    if (
      !slug.match(
        /^(Hero|Talent|Inventory|Legendary|Recipe|Pactspirit|Craft|Black|Active|Support|Passive|Netherrealm|Destiny|Enchant|Corruption|Dream|Tower|God|Profession)/i,
      )
    ) {
