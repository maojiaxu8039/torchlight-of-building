    ...
    while ((rowMatch = rowRegex.exec(enHtml)) !== null) {
      ...
      if (cells.length >= 3) {
        ...
        if (affixText && affixType && !affixText.includes("Affix Effect")) {
          ...
          enById[id] = { affix: affixText, type: affixType, pool: getCraftingPool(cells[2]) };
        }
      }
    }
...
async function main() {
  ...
  for (let i = 0; i < equipmentList.length; i++) {
    ...
    console.log(result.affixes.length + " 词缀, " + Object.keys(result.translations).length + " 翻译");
    ...
    if (result.affixes.length > 0) {
      ...
      Object.entries(byType).forEach(function(entry) {
        ...
        // 生成文件名
        const fileName = equipment.type.toLowerCase() + "-" + affixType.toLowerCase().replace(/ /g, "-") + ".ts";
        
        // 生成 TypeScript 内容
        const exportName = equipment.type.toUpperCase().replace(/-/g, "_") + "_" + affixType.toUpperCase().replace(/ /g, "_") + "_AFFIXES";
  ...
  console.log("翻译文件: src/data/translated-affixes/merged-all-translations.json");
