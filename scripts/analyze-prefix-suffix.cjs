...
async function main() {
  ...
  if (byType["Prefix"]) {
    const maxLifeVariants = byType["Prefix"].filter(function(text) {
      return text.includes("Max Life") && text.match(/\+\(\d+-\d+\)/);
    });
    
    console.log("Max Life 变体数量: " + maxLifeVariants.length);
    maxLifeVariants.forEach(function(v) {
      console.log("  " + v);
    });
  }
  ...
  if (byType["Prefix"]) {
    const templates = {};
    byType["Prefix"].forEach(function(text) {
      // 移除数值范围
      const template = text.replace(/\+\(\d+-\d+\)/g, "+(--)").replace(/\+\d+-\d+/g, "+--");
      if (!templates[template]) templates[template] = 0;
      templates[template]++;
    });
    
    console.log("唯一模板数量: " + Object.keys(templates).length);
    Object.entries(templates).slice(0, 10).forEach(function(entry) {
