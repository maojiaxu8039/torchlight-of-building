...
function extractAffixName(
  enText: string,
  cnText: string,
): { en: string; cn: string } | null {
...
function extractAllAffixNames(): Map<string, string> {
  const inputData: UniqueTranslations = JSON.parse(
...
function generateTypeScriptFile(affixNames: Map<string, string>): void {
  const sortedEntries = Array.from(affixNames.entries()).sort((a, b) =>
...
`,
  ...
  console.log(
    `Generated stat word translations: ${outputFile} (${sortedStatWords.length} entries)`,
  );
}
