...
function extractCraftAffixes(html: string): Map<string, { text: string; equipmentType: string; affixType: string }> {
  const $ = cheerio.load(html);
  const affixes = new Map<string, { text: string; equipmentType: string; affixType: string }>();
...
async function scrapeCraftAffixes(): Promise<void> {
  ...
  const uniqueOutputFile = path.join(outputDir, 'craft-unique-affix-translations.json');
  fs.writeFileSync(
    uniqueOutputFile,
...
`;
  ...
  console.log(`Affix types: ${Array.from(new Set(translations.map(t => t.affixType))).join(', ')}`);
}
