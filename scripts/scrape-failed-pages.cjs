...
function extractTranslationsFromHtml(html) {
  ...
  while ((match = tablePattern.exec(html)) !== null) {
    ...
    if (tds.length >= 2) {
      // 获取最后两列作为 EN/CN 对
      const texts = tds.map(td => td.replace(/<[^>]+>/g, ' ').replace(/&nbsp;/g, ' ').replace(/&ndash;/g, '–').replace(/\s+/g, ' ').trim());
      const lastText = texts[texts.length - 1];
      const secondLastText = texts[texts.length - 2];
      if (lastText && secondLastText && lastText !== secondLastText) {
        translations['table_' + Object.keys(translations).length] = { en: secondLastText, cn: lastText };
      }
    }
  }
...
async function scrapePage(enUrl, cnUrl, pageName) {
  try {
    ...
    return { pageName, count, status: 'success', translations: matchedTranslations };
...
async function main() {
  ...
  const pagesWithModifier = [
    'Confusion_Card_Library',
    'Void_Chart',
    'Compass',
  ];
