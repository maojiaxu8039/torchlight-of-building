...
// 读取网页模板翻译
const webTranslations = JSON.parse(fs.readFileSync("src/data/translated-affixes/merged-all-translations.json", "utf8"));

// 生成模板翻译
const templates = {};

Object.entries(webTranslations).forEach(function(entry) {
  const en = entry[0];
  const cn = entry[1];
...
// 读取游戏数据
const gameFiles = fs.readdirSync("src/data/gear-affix").filter(f => f.endsWith(".ts"));
...
gameFiles.forEach(file => {
  ...
  if (file.includes("-prefix") || file.includes("-suffix")) {
    ...
    if (affixMatches) {
      ...
      affixMatches.forEach(function(m) {
...
// 保存翻译
const sorted = Object.entries(webTranslations).sort(function(a, b) { return b[0].length - a[0].length; });
const result = {};
