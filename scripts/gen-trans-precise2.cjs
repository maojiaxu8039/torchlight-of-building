...
function parseAffixes(html) {
  ...
  while ((idMatch = idRegex.exec(html)) !== null) {
    ...
    text = text
      ...
      .replace(/<a[^>]*>([^<]*)<\/a>/gi, '$1')
      .replace(/<img[^>]*>/gi, '')
      .replace(/<span[^>]*class="text-mod"[^>]*>/gi, '【TEXT_MOD_START】')
      .replace(/<span[^>]*>/gi, '')
      .replace(/<\/span>/gi, '【TEXT_MOD_END】')
      .replace(/<[^>]+>/g, '')
      .replace(/&nbsp;/g, ' ')
      .replace(/&ndash;/g, '-')
      .replace(/&amp;/g, '&')
    ...
    // 只保留 text-mod 内容之间的文本
    const textModMatch = text.match(/【TEXT_MOD_START】([\s\S]*?)【TEXT_MOD_END】/);
    if (textModMatch) {
      text = textModMatch[1].trim();
    } else {
      // 如果没有 text-mod，只取前面的内容
      const parts = text.split('【TEXT_MOD_END】');
      text = parts[0].trim();
    }
    ...
    text = text.replace(/\s*[\u4e00-\u9fa5]{1,10}(?:等级|词缀|序列|加工|进阶|初阶|高阶)[^"]*$/g, '').trim();
