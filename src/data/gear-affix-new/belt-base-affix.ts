// Auto-generated from tlidb.com
import type { BaseGearAffix } from "../../tli/gear-data-types";

export const BELT_BASE_AFFIX_AFFIXES: readonly BaseGearAffix[] = [
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(54-74) Max Life",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(40-60) Max Mana",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(87-117) Max Energy Shield",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(5-10)% Fire Resistance",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(5-10)% Cold Resistance",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(5-10)% Lightning Resistance",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(5-10)% Erosion Resistance",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(8-10)% Movement Speed",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(20-25)% Cooldown Recovery Speed",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(20-25)% Skill Effect Duration",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(6-8)% Sealed Mana Compensation",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "+(7-8)% Aura Effect",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "Immune to ParalysisImmune to Blinding",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "Immune to SlowImmune to Weaken",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "Restoration Skills gain 1.5 Charging Progress every second",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "Gains 1 stack(s) of all Blessings when casting a Restoration Skill",
  },
  {
    equipmentSlot: "Trinket",
    equipmentType: "Belt",
    affixType: "Base Affix",
    craftingPool: "Basic",
    tier: "1",
    craftableAffix: "Has Hasten",
  },
] as const;
